...
def getArgs() :
    ...
    parser.add_argument("--cuts", choices=["tight","loose"], default="loose", help="Apply tight or loose object selection")
...
for count, e in enumerate(inTree) :
    ...
    for isyst, systematic in enumerate(sysT) : 
        ...
        elif args.cuts == "loose":
            goodElectronList = TF.makeLooseElectronListDCH(e, cutflow['ele'])
            goodMuonList     = TF.makeLooseMuonListDCH(e, cutflow['muon'])
            goodTauList      = TF.makeLooseTauList(e, cutflow['tau'])
