...
def process_file(year, sample, rf):
...
def run_jobs_parallel(max_workers=4):
    tasks = []
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        for year, samples in jobs.items():
            for sample, files in samples.items():
                for rf in files:
                    tasks.append(executor.submit(process_file, year, sample, rf))

        for future in as_completed(tasks):
            try:
                future.result()
            except subprocess.CalledProcessError as e:
                print(f"[ERROR] Job failed: {e.cmd}")
            except Exception as e:
                print(f"[ERROR] Unexpected: {e}")

if __name__ == "__main__":
    run_jobs_parallel(max_workers=8)  
